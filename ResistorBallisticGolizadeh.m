...
% TOL - controls outer loop accuracy
% epsilon - controls inner iteration loop
%
% Syntax:  ResistorBallisticGolizadeh
%
% Outputs:
...
% Subfunctions: none
% MAT-files required: Upot.mat (potential profile generated by ResistorBallisticKzero.m )
...
% Initial potential profile (zero)
    ...
    % Load previously calculated potential
    load 'Upot.mat'  
...
% Initialize Green's functions
  ...
  G_les         = zeros(Nx,Nx,NK);    % lesser (electron) GF
  BSE_L         = zeros(Nx,Nx,NK);    % boundary self-energy left
  BSE_R         = zeros(Nx,Nx,NK);    % boundary self-energy right
  BSE_les       = zeros(Nx,Nx,NK);    % boundary self-energy lesser
% Scattering  
  SE_G          = zeros(Nx,Nx,NK);    % Golizadeh self-energy right
  SE_Gles       = zeros(Nx,Nx,NK);    % Golizadeh self-energy lesser
  ...
  %%%
  SE_Gles_TE   =  zeros(Nx,NE);           % Golizadeh lesser total integrated
...
for EnergyNode=1:NE
  ...
  Step_in = 1;
   ...
   while epsilon>0.01
      ...
      fprintf('Main Cycle %d Energy cycle %d Inner cycle %d and convergence %d  \n',Step, EnergyNode, Step_in, TOL);
      Step_in=Step_in+1;
 
 % Analytic calculation of k_L, k_R
 k_L(1:NK) = (1/(dx)).*acos(  1 - m_e*dx^2*(E(EnergyNode) - E_C - E_k(1:NK) - U(1) - SE_GL(1:NK)  )/(h*hh)); 
 k_R(1:NK) = (1/(dx)).*acos(  1 - m_e*dx^2*(E(EnergyNode) - E_C - E_k(1:NK) - U(Nx)- SE_GR(1:NK)  )/(h*hh));
 % Surface Green's functions
 sgf_L(1:NK) = - t*exp(1i*k_L(1:NK)*dx);
 sgf_R(1:NK) = - t*exp(1i*k_R(1:NK)*dx);
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 % Boundary self-energies 
 BSE_L(1,1,1:NK)    = sgf_L(1:NK);
 BSE_R(Nx,Nx,1:NK)  = sgf_R(1:NK);
 ...
 for k=1:NK
    BSE_les(:,:,k)    =   1i * (  GamL(:,:,k)*f0L(EnergyNode)  +  GamR(:,:,k)*f0R(EnergyNode) );            
  end
 ...
 G_les_T       =  zeros(Nx,Nx);  
 G_R_T         =  zeros(Nx,Nx); 
 SE_Gles_T      =  zeros(Nx,Nx);
 
 % Green's function calculation for all k-points
 for k=1:NK

  G_R(:,:,k) = inv( (E(EnergyNode)+gamm)*eye(Nx) - T(:,:,k) - diag(U) - BSE_L(:,:,k) - BSE_R(:,:,k) - SE_G(:,:,k));
  G_les(:,:,k) = G_R(:,:,k)*(BSE_les(:,:,k) + SE_Gles(:,:,k))*G_R(:,:,k)';
  % integrated over k quantities
  G_les_T = G_les_T + 1/pi*dk*k_point(k)*G_les(:,:,k);
  G_R_T = G_R_T + 1/pi*dk*k_point(k)*G_R(:,:,k);
  
 end
      
      epsilon = abs(max(G_R(:) - G_R_old(:))/abs(max(G_R_old(:))));
      
    % scattering self-consistent loop  
      for k=1:NK
         SE_G(:,:,k)    =  d*diag(diag(G_R(:,:,k)));
         SE_Gles(:,:,k) =  d*diag(diag(G_les(:,:,k)));
         SE_GL(k) = SE_G(1,1,k);
         SE_GR(k) = SE_G(Nx,Nx,k);
         % k-integrated 
         SE_Gles_T = SE_Gles_T + 1/pi*dk*k_point(k)*SE_G(:,:,k);
      end
         G_R_old = G_R;
 ...
 % Electron density (total)
  ...
  n  = n - real(1i*DE/(2*pi*dx)*diag(G_les_T))'; 
 

 % Spectral electron density 
 
  ElDen(:,EnergyNode) = - real(1i/dx*diag(G_les_T))';
  % E-integrated GSE_lesser
  SE_Gles_TE(:,EnergyNode) = (1/dx*diag(SE_Gles_T))';
 ...
 % Spectral current
         for SpaceGrid=1:Nx-1
                 J_C(SpaceGrid, EnergyNode) = J_C(SpaceGrid,EnergyNode) + (q/h)*(W*G_les_T(SpaceGrid+1,SpaceGrid) - ... 
                           W*G_les_T(SpaceGrid,SpaceGrid+1));                     
         end
...
% 3D-LDOS at point x plot vs analytic 
x=20;
...
% figure('Name','Golizadeg SE<','NumberTitle','off');
% colormap(hot);
% imagesc(abs(SET(:,:)'))
